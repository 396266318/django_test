...
class LoginActionTest(TestCase):
	def setUp(self):
		User.objects.create_user("test01", "test@mail.com", "test123456")
		self.client = Client()
	...
	def test_login_null(self):
		...
		self.assertEqual(response.status_code, 200)
		self.assertEqual("用户名或者密码为空", login_html)
	...
	def test_login_error(self):
		...
		login_html = response.content.decode("utf-8")
		self.assertEqual(response.status_code, 200)
		self.assertEqual("用户名或者密码错误", login_html)
