...
def case_manage(request):
	...
	paginator = Paginator(testcases, 2)  # 页面列表显示数据条数
...
def search_case_name(request):
	if request.method == "GET":
		...
		cases = TestCase.objects.filte(name__contains=case_name)
...
def get_case_info(request):
	if request.method == "POST":
		...
		case_info = {
			...
			"rea_header": case_obj.req_header,
