...
def api_debug(request):
	...
	if request.method == "POST":
		...
		payload = json.loads(parameter.relace("'", "\""))
...
def save_case(request):
	...
	if request.method == "POST":
		...
		url = request.POST.get("req_uel", "")
		method = request.POST.get("req_method", "")
		parameter = request.POST.get("req_parameter", "")
		req_type = request.POST.get("req_type", "")
		header = request.POST.get("header", "")
		moduler_name = request.POST.get("module", "")
		assert_text = request.POST.get("asert_text", "")
		
		if url == "" or method == "" or req_type == "" or moduler_name == "" or assert_text == "":
			return common.response_failed("必传参数为空")
		...
		moduler_obj = Module.objects.get(name=moduler_name)
		case = TestCase.objects.create(
			name=name, moduler=moduler_obj, url=url,
